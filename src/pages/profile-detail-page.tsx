...
const SKILL_LEVELS: Record<string, { label: string; pct: number }> = {
  React: { label: "Expert", pct: 92 },
  "Node.js": { label: "Expert", pct: 88 },
  PostgreSQL: { label: "Avancé", pct: 75 },
  AWS: { label: "Intermédiaire", pct: 60 },
  TypeScript: { label: "Expert", pct: 90 },
  Python: { label: "Avancé", pct: 78 },
  Docker: { label: "Avancé", pct: 72 },
  "Vue.js": { label: "Intermédiaire", pct: 65 },
};

function getSkillLevel(tech: string) {
...
export function ProfileDetailPage() {
    ...
    async function fetchData() {
      try {
        ...
        if (profileData) {
          ...
          // Supabase returned nothing — check mock profiles
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 pb-24 pt-4 md:pb-8">
      {/* Back */}
      <Link to="/contributeurs">
        <Button variant="ghost" size="sm" className="mb-5 gap-2 text-muted-foreground hover:text-foreground hover:bg-white/5">
          <ArrowLeft className="h-4 w-4" />
          Retour
        </Button>
      </Link>
        ...
        <aside className="flex flex-col gap-4">
          ...
          {profile.bio && (
            <div className="glass-panel rounded-2xl border border-white/10 p-5">
              <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">À propos</h3>
              <p className="text-sm leading-relaxed text-foreground/80">{profile.bio}</p>
            </div>
          )}

          {/* Technical Arsenal — skill bars */}
          {profile.tech_stack.length > 0 && (
            <div className="glass-panel rounded-2xl border border-white/10 p-5">
              <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Arsenal Technique</h3>
              <div className="space-y-3">
                {profile.tech_stack.slice(0, 6).map((tech) => {
                  const skill = getSkillLevel(tech);
                  return (
                    <div key={tech}>
                      <div className="mb-1 flex items-center justify-between">
                        <span className="text-xs font-medium text-foreground">{tech}</span>
                        <span className="text-[10px] text-muted-foreground">{skill.label}</span>
                      </div>
      ...
      <div className="lg:hidden">
        ...
        <div className="glass-panel rounded-2xl border border-white/10 p-5 mb-4">
          ...
          <div className="mt-4 grid grid-cols-3 gap-2">
            <div className="rounded-lg border border-white/10 bg-white/5 py-2 text-center">
              <p className="text-base font-bold text-foreground">{repos.length}+</p>
              <p className="text-[10px] text-muted-foreground">Repos</p>
            </div>
            <div className="rounded-lg border border-white/10 bg-white/5 py-2 text-center">
              <p className="text-base font-bold text-foreground">{repos.reduce((s, r) => s + (r.stars || 0), 0)}</p>
              <p className="text-[10px] text-muted-foreground">Étoiles</p>
            </div>
            <div className="rounded-lg border border-white/10 bg-white/5 py-2 text-center">
              <p className="text-base font-bold text-foreground">{profile.tech_stack.length}</p>
        ...
        {activeTab === "biographie" && (
          <div className="space-y-3">
            ...
            <div className="glass-panel rounded-xl border border-white/10 p-4">
              <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Localisation</h3>
            ...
            {profile.github_url && (
              <div className="glass-panel rounded-xl border border-white/10 p-4">
                <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Liens</h3>
                <a href={profile.github_url} target="_blank" rel="noopener noreferrer"
                  ...
                  return (
                    <div key={tech}>
                      <div className="mb-1.5 flex items-center justify-between">
                        ...
                        <span className="text-[11px] text-muted-foreground">{skill.label} — {skill.pct}%</span>
                      </div>
            ...
            {repos.length === 0 ? (
              ...
              repos.map((repo) => (
                <a key={repo.id} href={repo.url} target="_blank" rel="noopener noreferrer"
                  ...
                  {repo.description && (
                    <p className="mb-2 text-xs text-muted-foreground line-clamp-2">{repo.description}</p>
                  )}
                  <div className="flex items-center gap-3 text-[11px] text-muted-foreground">
                    {repo.language && (
                      <div className="flex items-center gap-1">
